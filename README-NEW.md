...
Для простого понимания `node` можно воспринимать как искусственный нейрон.

- Нода хранит смысл и контекст.
- Нода может активироваться по триггерам, когда агенту нужен именно этот кусок знаний.
- Связи `[[...]]` между файлами можно понимать как синапсы: через них знания связываются и передают контекст.

Иначе говоря, здесь файл нужен не только для хранения текста, но и как рабочая единица мышления и поведения агента. При этом в именовании мы сохраняем текущий технический формат `*.node.md`.
