...
Они полезны, но не обязательны. Для них действует **тот же** YAML-контракт, что и для любой другой `*.node.md`: **`AWN-TYPE: CoreNode`** и тот же набор полей; роль задаёт **имя файла**, а не отдельное значение `AWN-TYPE`. Рекомендации по `AWN-REQUIRE` / `AWN-TRIGGERS` и примеры скелетов — в `AGENTS.md` (подраздел про `Index.node.md` и `Area.node.md`).
...
- `AWN-TYPE` — тип файла. Для нод используется `CoreNode`.
...
Отдельные файлы с расширением **`.memory.md`** (не `.mem.md` — так однозначнее в поиске) хранят память для ноды с `AWN-MEMORY: external` или `hybrid`. В шапке — `AWN-TYPE: MemoryRecord`, связь с нодой через `AWN-PARENT-NODE`, теги в поле **`TAGS`**, статус и даты; в реестр `AGENTS.NODES.md` эти файлы не входят. Полный список полей — в `AGENTS.md`.
