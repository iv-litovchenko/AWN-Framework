...
Внутри папок показаны **типичные** ноды: `Area.node.md` — что за область и что здесь «живёт»; `Index.node.md` — по желанию вход в папку. В корне экосистемы удобно держать **паспорт агента** и **профиль пользователя** отдельными нодами.

```text
vault/
├── 00 🍀 Aya.AI/                  # ядро: правила экосистемы
│   ├── Area.node.md               # что в этой папке и зачем она (контекст)
...
Имён файлов в подпапках может быть **сколько угодно** или **ни одного**: `Area.node.md` / `Index.node.md` не обязательны ([`AGENTS.md`](AGENTS.md) §4). Достаточно одной `Area.node.md` на папку или только «контент-нод» без обложки области — как удобнее **тебе**.
