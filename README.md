...
- агент по задаче может **создавать и обновлять YAML-свойства (frontmatter)** в нужных `.md` (ноды, `*.memory.md` и др.) и **поддерживать реестр** `AGENTS.NODES.md`, когда меняются поля, влияющие на таблицу (подробнее в `AGENTS.md`, раздел **«Основные команды»**).
...
- `Area.node.md` — **оболочка области** («мир»): смысл и границы **крупной** зоны (финансы, здоровье и т.д.), не обязательно одна папка;
...
- `AWN-TYPE` — тип файла. Для нод: `NODE/AREA`, `NODE/INDEX` или `NODE/SOLO`; для `*.memory.md`: `MEMORY`.
...
- `AWN-MEMORY` — режим памяти: `none` (нет выделенной памяти), `internal` (журнал внутри этой ноды), `external` (записи в `*.memory.md`), `hybrid` (и то и другое). Подробно и минимальный контракт файлов памяти — в `AGENTS.md` (раздел «Память ноды»).

**Свои поля** (расширения под задачу или vault): любые другие ключи в той же YAML-шапке — **без** префикса `AWN-`, имя ключа **на английском** (`tags`, `parentNote`, `locale` и т.д.); не совпадать с каноническим списком выше и не начинаться с `AWN-` (см. `AGENTS.md`, раздел «Конвентция имён свойств в YAML»).

### Внешняя память: `*.memory.md`

Отдельные файлы с расширением **`.memory.md`** (не `.mem.md` — так однозначнее в поиске) хранят память для ноды с `AWN-MEMORY: external` или `hybrid`. В шапке сначала **`AWN-PARENT-NODE`** (путь к владеющей `*.node.md`), затем **`AWN-TYPE: MEMORY`**, **`TAGS`**, статус и даты; в реестр `AGENTS.NODES.md` эти файлы не входят. Полный список полей и порядок — в `AGENTS.md`.
