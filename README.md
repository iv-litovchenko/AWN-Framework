...
Отдельные файлы с расширением **`.memory.md`** (не `.mem.md` — так однозначнее в поиске) хранят память для ноды; у родительской ноды часто ставят `AWN-MEMORY: external` или `hybrid` как **напоминание** (поле справочное). В шапке сначала блок **`AWN-*`**: **`AWN-PARENT-NODE`**, **`AWN-TYPE: MEMORY`**, **`AWN-STATUS`**, **`AWN-CREATED`**, **`AWN-UPDATED`**; затем **`TAGS`** и другие ключи **без** `AWN-` (включая полностью пользовательские). В реестр `AGENTS.NODES.md` эти файлы не входят. Шапка и markdown-тело — **один файл**; в `AGENTS.md` пример оформлен **одним блоком**: `yaml`-шапка и сразу строка про тело. Полный контракт — там же (раздел «Память ноды»).
