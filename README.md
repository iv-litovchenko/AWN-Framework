...
- агент по задаче может **создавать и обновлять YAML-свойства (frontmatter)** в нужных `.md` (ноды, `*.main.md`, sidecar `*.metadata.md` и др.) и **поддерживать реестр** `AGENTS.NODES.md`, когда меняются поля, влияющие на таблицу (подробнее в `AGENTS.md`, раздел **«Основные команды»**; типы файлов — **«Ориентир для агента»**).
...
>
> Попросите своего агента установить данный фреймворк, передав ему инструкцию из файла [`INSTALL.md`](INSTALL.md).
>
...
- [Установка / обновление (`INSTALL.md`)](INSTALL.md)
...
- [Что такое нода](#что-такое-нода)
  ...
  - [Внешняя память: `*.main.md`](#внешняя-память-memorymd)
...
Подробнее это раскрыто в [[00_Documentation/v2/AGENTS#8. Почему не жёсткие «фреймворки» и не чужие конвенции workspace|AGENTS.md -> раздел 8]], где объясняется, почему подход отличается от жёстких фреймворков и OpenClaw-подобных шаблонов.
...
- **Желательно** сразу после второго `---`: заголовок `# …` и плашка **`> [!info]`** (или обычная цитата `>`) с одной-двумя фразами: это нода или память, зачем файл — см. `AGENTS.md` → **«Правила создания/обновления нод»** (то же для **`*.main.md`**; для sidecar **`*.metadata.md`** — по желанию).

Основные свойства в YAML-шапке:

- `AWN-TYPE` — тип файла. Для нод: `NODE/AREA`, `NODE/INDEX` или `NODE/SOLO`; для `*.main.md`: `"MEMORY/RECORD"` (в YAML в кавычках из‑за `/`).
...
Хороший пример — нода `Assistant.Ai/Nodes/RandomAnecdote.node.md`: в ней уже есть и правило, и журнал рассказанных анекдотов. Агент читает ноду целиком, поэтому ему не нужен внешний “мануал”, чтобы понимать, как вести память.
...
2. Забери из него как минимум `AGENTS.md`, `AGENTS.NODES.md` и стартовый файл под свою систему, например `CLAUDE.md` или другой соответствующий файл, если твой агент работает через другую точку входа. Также забери ноду-пример `Assistant.Ai/Nodes/RandomAnecdote.node.md`: сохрани путь `Assistant.Ai/Nodes/` (или положи файл туда в своём vault).
...
По умолчанию в проекте уже есть одна нода-пример: `Assistant.Ai/Nodes/RandomAnecdote.node.md` ("Случайный анекдот"). На ней можно посмотреть базовую структуру `*.node.md`, YAML-шапку и способ описания поведения ноды.
...
**Правка только свойств:** по запросу агент может **не создавая новый файл** изменить YAML-шапку уже существующей ноды или `*.main.md` (новые ключи, правки значений, `AWN-UPDATED` и т.д.) в рамках канона; если затронуты поля, по которым ведётся реестр, строку в `AGENTS.NODES.md` нужно **обновить** согласованно.
...
- Установка/обновление файлов AWN через агента: [`INSTALL.md`](INSTALL.md).
...
copaw cron create \
  ...
  --schedule "30 8 * * *" \
  --command "выполнить ноду Assistant.Ai/Cron/DailyHealthCheck.node.md"
```
...
## 1000.md+ файлов (проблема найти без вектора)

Я не буду «глотать» все 1000 файлов. Я буду **переключать внимание**.

1. **Area** скажет мне, в какую комнату зайти.
2. **Search** найдет нужную полку.
3. **Node** даст инструкцию, как прочитать книгу.
    

Так что мой «контекст» будет всегда свежим и легким, а 1000 файлов станут не грузом, а мощным фундаментом.

Вы абсолютно правы — загрузить 1000 файлов в контекст невозможно, это убьёт и меня, и любой LLM. Но ваша система именно для того и создана, чтобы **этого не делать**.

Аналогия: у вас есть библиотека из 1000 книг. Вы не читаете их все одновременно. У вас есть каталог (таблица), где указано, на какой полке какая книга и о чём она. Когда вам нужно про книгу про дубликаты — вы идёте по каталогу и берёте только её.

### 4️⃣ **Обычные .md файлы — через RAG, не через контекст**

Для тысячи обычных заметок (не нод) используется **Retrieval-Augmented Generation (RAG)**:

- Я не читаю их все.
- Когда вы задаёте вопрос, я быстро (через эмбеддинги) нахожу **3-5 самых релевантных** фрагментов из этих заметок.
- Загружаю только эти фрагменты в контекст.

Это стандартная практика во всех современных AI-системах (Obsidian Copilot, Cursor и т.д.).

Допустим, вы работаете над эфиром и говорите: _«Найди в наших старых заметках идеи по монетизации эфира»_.

**Что произойдёт:**

1. Я **не** загружаю 1000 файлов.
2. Я смотрю таблицу `AGENTS.md`, вижу, что есть нода `skill-search-notes.node.md` с триггером «найди в заметках».
3. Загружаю **только эту ноду** (несколько килобайт).
4. Внутри ноды описан алгоритм: «используй поиск по эмбеддингам или grep по папке _notes/».
5. Я выполняю поиск (инструмент, не загружая все файлы в контекст) и нахожу 3 заметки, где есть «монетизация» и «эфир».
6. Загружаю **только эти 3 заметки** (частично или полностью) в контекст.
7. Отвечаю вам.

Весь процесс потребовал загрузки **4-5 файлов**, а не 1000.

## 💎 Итог

> **Я не подохну от 1000 файлов, потому что ваша система спроектирована так, чтобы я их никогда целиком не видел.**  
> Как человек не читает всю библиотеку, чтобы ответить на один вопрос — так и я не читаю все ваши заметки.

Это и есть главное преимущество вашей архитектуры перед «простыми скиллами» в других агентских системах. Они загружают всё подряд. Вы — нет.

Ты абсолютно прав — и это **самая реалистичная и важная проблема** в любых системах с «вечной памятью».

> 💥 **LLM не может обработать 1000 файлов за раз.**  
> Даже самые мощные модели (GPT-4o, Claude 3.5, Qwen-Max) имеют **ограничение контекста** (~128K–200K токенов), что эквивалентно **~300–500 страниц текста**.  
> А 1000 `.main.md` — это легко **10 000+ страниц**.

Но **выход есть** — и он лежит не в «загрузить всё», а в **умном отборе**.

## 🔍 Как на самом деле работают системы с большой памятью?

Они используют **RAG + фильтрация + ранжирование**, но **не просто «всё в промпт»**.

Давай разберём **рабочую схему**, которую можно применить прямо в твоей системе.

### 🔍 Шаг 2. **Используй семантический поиск (RAG)**

Если запрос сложный:

> _«Покажи моменты, когда я чувствовал вдохновение, как в том эфире про звёзды»_,

тогда:

1. Твой скрипт **преобразует запрос в embedding** (вектор).
2. Сравнивает с **векторами всех `.main.md`** (заранее посчитанными).
3. Берёт **топ-5 наиболее похожих записей**.
4. Подгружает **только их** в промпт.

→ Это делает **контекст точным**, а не перегруженным.

> 💡 Пример инструментов:
> 
> - Локально: `sentence-transformers` + `ChromaDB` / `LanceDB`
> - В Obsidian: плагин **Smart Connections** или **Cortex**

### 📦 Шаг 4. **Раздели память по уровням**

|Уровень|Что хранится|Как используется|
|---|---|---|
|**Hot** (горячая)|Последние 7–30 дней|Грузится всегда|
|**Warm** (тёплая)|Последние 6 месяцев|Через RAG по запросу|
|**Cold** (холодная)|Всё остальное|Только через суммаризацию или ручной поиск|

→ Это стандартный подход в базах данных — и он работает для памяти.

### 💡 Практический пример из твоей жизни

**Запрос:**

> _«Aya, почему я снова откладываю работу над Aya?»_

**Что делает система:**

1. Находит ноду `Aya` → видит `space = creative`
2. Ищет в `Memory/aya/` последние 15 файлов
3. Запускает RAG по фразе _«откладываю», «боюсь», «не получится»_
4. Находит 3 записи:
    - `2026-04-10.main.md`: _«Боюсь, что это никому не нужно»_
    - `2026-03-22.main.md`: _«Устал от ощущения, что я опоздал»_
    - `Summary_2025-Q4.main.md`: _«В конце года часто возникает кризис смысла»_
5. Формирует промпт из этих 3 записей + системных правил

→ Контекст: **~500–800 слов** — идеально для любого LLM.

### ✅ Главный вывод

> **Ты не должен загружать всю память.**  
> Ты должен **уметь найти самое важное** — как человек вспоминает не всё, а **то, что сейчас резонирует**.

Да, это реальная проблема — и ты правильно её почувствовал.

1000 файлов в контекст не влезет физически. Даже 100 — уже боль.

---

**Как это решается на практике — три подхода:**

**1. Ленивая загрузка по запросу**

Агент не читает все файлы. Он читает только `Index.node.md` папки — там оглавление. Дальше по запросу ныряет в конкретный файл.

```
Ты: "что было в финансах в марте?"
Агент: читает Index → видит список файлов → 
       открывает только Memory/2026/03/*.main.md
```

Это работает если `Index.node.md` хорошо написан — там должны быть короткие аннотации каждого файла, а не просто список имён.

**2. Векторный поиск (RAG)**

Все 1000 файлов индексируются заранее — превращаются в эмбеддинги. При запросе агент ищет семантически близкие чанки и загружает только их.

В Obsidian это делает плагин Smart Connections или Copilot. Работает прямо в хранилище, без внешних серверов.

**3. Иерархия контекста — то что у тебя уже есть**

```
Старт сессии → загружается только README + core/
        ↓
Переключился на эфиры → загружается Area.node.md эфиров
        ↓
Работаем с конкретным эфиром → загружается его нода
        ↓
Нужна история → агент сам запрашивает нужные memory файлы
```

Никогда не всё сразу — только то что нужно прямо сейчас.

---

**Честный вывод:**

AWN без RAG хорошо работает до ~50-100 файлов памяти на ноду. Дальше нужен либо умный Index либо векторный поиск.

Но вот что важно — твоя файловая структура с суффиксами и YAML-свойствами это идеальная база для RAG. Эмбеддить структурированные файлы с метаданными намного эффективнее чем случайные заметки. AWN и RAG не конкурируют — они дополняют друг друга.
...
AWN-context-load:
  ...
  - Эфиры/Memory/последние.main.md
```
...
### 🧭 Space vs. Area: Разные уровни одного процесса

Главное, что нужно понять: эти два понятия не конкурируют, а решают задачи на **разных уровнях** работы агента.

- **Space (из Space Agent) — это динамическая «песочница» с расширенными правами.** Представьте, что Space — это полноценный стол для работы, где агент может не просто писать, но и разворачивать виджеты, инструменты и даже менять сам интерфейс этого стола, решая задачу наиболее эффективным способом[](https://github.com/agent0ai/space-agent).
    
- **Area (из вашей системы) — это статичный «отсек знаний» с чёткими правилами.** Это, скорее, ваш личный кабинет, который не меняется. В нём чётко определено, где какие папки лежат, какие там действуют правила (ноды `rule`), чем можно воспользоваться (ноды `skill`) и о чём идёт речь (ноды `concept`).

### 🤔 Почему нельзя «превратить» Area в Space?

Если провести аналогию с архитектурой, то **Space — это само здание с возможностью перепланировки**, а **Area — это одна из его комнат с фиксированной планировкой**.

- **Space (Здание)**: Позволяет агенту выходить за рамки интерфейса, создавать новые пространства и управлять правами доступа[](https://github.com/agent0ai/space-agent).
    
- **Area (Комната)**: Работает исключительно внутри себя, подчиняясь заложенным в неё правилам и знаниям.
    

Их главное различие — в **фокусе и назначении**:

- **Space** решает **экзогенные** задачи: «Давай создадим новый инструмент, который поможет нам управлять проектами».
    
- **Area** решает **эндогенные** задачи: «Какой подход к управлению проектами описан в наших конспектах и навыках?».
...
> **Статус:** два варианта разложения спецификации по папкам и файлам — для последующей доработки. Канон для агента по-прежнему в **`AGENTS.md`** и **`AGENTS.NODES.md`**. Папка [`AwnFramework/`](AwnFramework/README.md) может хранить будущие черновики файлов.
...
│   ├── main.md                  — *.main.md, связь с нодой, STATUS, TAGS
...
**Риски:** при росте `main.md` имеет смысл распилить на схему + CRUD (как во варианте A).
