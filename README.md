...
> [!NOTE]
> Минимальная операционная система для агента. AI-native PKM, где агент является полноценным жителем системы, а не просто инструментом. Tana, Capacities, Logseq — они мощнее "из коробки", но закрывают тебя в своей экосистеме и форматах. AWN — наоборот: plain Markdown, никаких проприетарных баз, агент читает то же, что и пользователь. Это даёт **портативность и прозрачность**, но требует **ручной дисциплины**. Попытка построить **протокол симбиоза человека и агента**. Тут не просто "как хранить заметки", а "как договориться о работе".
...
>
> Попросите агента скачать (скопировать) следующие файлы из репозитория в корень проекта и передайте ему инструкцию ниже.
