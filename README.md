...
```text
Контекст задачи:
Ты выполняешь установку AWN Framework в текущий проект пользователя.
...
Не сама идея хранить память в файлах — это старо. Новое это **явный контракт** (`TYPE`, `REQUIRE`, `TRIGGERS`, `PRIORITY`) который делает систему предсказуемой для агента без сложной инфраструктуры. Большинство систем либо слишком жёсткие либо слишком свободные.
...
9. **Приоритеты подключаемых `Area`/`Index` и обычных нод** — доработать и явно зафиксировать, как учитывать `PRIORITY` для `Area.node.md` и `Index.node.md` при их подключении вместе с целевой нодой: всегда ли они загружаются безусловно по пути, участвуют ли в общей сортировке, и что делать при одинаковом приоритете.
...
12. **Режимы загрузки нод (концепт vs контракт `REQUIRE`)** — развести «как бывает на практике» и «что зашито в YAML». Четыре смысловых варианта подгрузки:
...
В поле **`REQUIRE`** сейчас зафиксированы только `start` и `on_demand`. **`explicit`** в enum не обязателен (YAGNI): команда вроде «загрузи ноду X» и так открывает файл. **`context`** — подбор по семантике диалога, а не по `TRIGGERS`; нужны эмбеддинги, индекс и политика приватности, без инфраструктуры режим остаётся концептом. **Итог:** при желании вынести таблицу короткой заметкой в `AGENTS.md` / `AGENTS.NODES.md` (дизайн-раздел), не расширяя `REQUIRE` без явной пользы.

13. **Стандарт Obsidian по оформлению YAML-свойств** — описать в `AGENTS.SETTINGS.md` (или рядом) единый стиль для шапок `*.node.md` и заметок: кавычки/мультистроки, порядок ключей, формат списков (`TRIGGERS` и др.), отступы — чтобы в vault и при просмотре на GitHub метаданные выглядели одинаково и их проще парсил агент.
