...
Отдельные файлы с расширением **`.memory.md`** (не `.mem.md` — так однозначнее в поиске) хранят память для ноды; у родительской ноды часто ставят `AWN-MEMORY: external` или `hybrid` как **напоминание** (поле справочное). В шапке сначала канонический блок: **`AWN-PARENT-NODE`**, **`AWN-TYPE: MEMORY`**, **`AWN-STATUS`**, **`TAGS`**, **`AWN-CREATED`**, **`AWN-UPDATED`**; **пользовательские** ключи без `AWN-` — после него. В реестр `AGENTS.NODES.md` эти файлы не входят. Полный контракт и пример — в `AGENTS.md` (раздел «Память ноды»).
