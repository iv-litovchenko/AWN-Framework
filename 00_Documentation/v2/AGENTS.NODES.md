...
Рабочий реестр `*.node.md` vault. Работает как **таблица маршрутов**: входящий запрос сопоставляется с `AWN-TRIGGERS` нод и определяет, какие файлы добавить в контекст агента. Файлы `*.main.md` и `*.metadata.md` в реестр **не** заносятся.
...
| active   | 20        | `Assistant.Ai/Nodes/MemoryFiles.node.md`               | Контракт файлов `*.main.md` — внешняя память нод           |
