...
function getTimeline(tasks) {
	...
	for (i=0; i<timelineDates.length; i++) {
		...
		if (timelineDates[i] == today) {
			...
			// Counter
			var todayContent = "<div class='todayHeader' aria-label='Focus today'></div>"
			...
			todayContent += "<div class='left'><select class='fileSelect' aria-label='Выберите заметку, куда добавить задачу'></select><input class='newTask' type='text' placeholder='Введите задачу'/></div>"
