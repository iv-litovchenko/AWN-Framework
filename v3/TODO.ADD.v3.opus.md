...
| Слой        | Тип     | Файл              | Где живёт                  | Что это                                                   |
| ----------- | ------- | ----------------- | -------------------------- | --------------------------------------------------------- |
| 0. Vault    | —       | —                 | корень                     | Корень хранилища                                          |
| 1. Cluster  | Cluster | `_index.node.md`  | `cluster-*/`               | Группа близких пространств                                |
| 1. Space    | Space   | `_index.node.md`  | `space-*/`                 | Самостоятельная область знаний                            |
| 2. Part     | Part    | `*.node.md`       | внутри Space               | Атомарная самостоятельная сущность                        |
| 3. Memory   | Note    | `*.note.md`       | `inbox/` или `notes/`      | Сырая входящая заметка (поток времени)                    |
| 3. Memory   | Page    | `*.page.md`       | `pages/`                   | Переработанное знание (страница книги)                    |
| 3. Memory   | Row     | `*.row.md`        | `databases/<type>/`        | Строка таблицы базы данных                                |
| 3. Memory   | Sidecar | `*.sidecar.md`    | рядом с не-md файлом       | Описание не-md файла (медиа, документ, бинарь)            |
| 3. Memory   | Volume  | `*.volume.md`     | рядом с родительской нодой | Приватная память ноды (грузится по запросу)               |
| 4. View     | View    | `*.view.md`       | `views/<type>/`            | Проекция над слоями 1–3, 5 (list/kanban/dashboard/mindmap)|
| 5. External | —       | любые             | в любой области            | Не-ноды: медиа, бинари, скрипты, конфиги, `*.md` без типа |

**Правила:**

- Cluster и Space различаются префиксом папки (`cluster-*/` vs `space-*/`), файл-якорь у обоих один — `_index.node.md`.
- Volume применим только к Space и Part: `<имя>.node.volume.md` или `_index.node.volume.md`. Остальные типы памяти Volume не имеют.
